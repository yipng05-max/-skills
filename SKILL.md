...
Chinese literature requires different verification strategies because most Chinese publications are not indexed in CrossRef. Follow this workflow:

### Chinese Journal Articles (中文期刊论文)

1. Run `scripts/search_cnki.py --title "<中文标题>"` to get search URLs and attempt CrossRef lookup
2. Use WebSearch to query `"<论文标题>" site:cnki.net` (知网)
3. Use WebSearch to query `"<论文标题>" site:wanfangdata.com.cn` (万方) for cross-validation
4. Use WebSearch to query `"<论文标题>" site:xueshu.baidu.com` (百度学术) as supplementary source
5. If the reference has a DOI, also run `scripts/verify_doi.py` — but note that missing DOI does NOT indicate fabrication for Chinese literature
...
- **`scripts/search_cnki.py`** — Generate search URLs for Chinese databases (CNKI, Wanfang, CQVIP, Baidu Scholar) and attempt CrossRef lookup. Use for Chinese literature verification.
